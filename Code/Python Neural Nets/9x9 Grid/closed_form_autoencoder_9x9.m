...
% for volalitility of 9x9 id_dfc18d626cbb42f1_data_price_norm_cutted
clearvars,clc;
%parpool(20);
load("id_dfc18d626cbb42f1_calibrationNN.mat")

params_real = params_real(:,[4,1,2,3,5]);
N = length(params_real);
interest_rate = rates;
params =params_NN(:,[4,1,2,3,5]);
...
% end
% 
% save("autoencoder_closedform_dfc18d626cbb42f1_results.mat","autoencoder_price","autoencoder_vola","bad_idx","real_price","real_vola")
%% Plots
% all
load("autoencoder_closedform_dfc18d626cbb42f1_results.mat")
figure
...
boxplot(rel_error)
subplot(3,5,2)
heatmap(reshape(mean(rel_error_price),9,9));
subplot(3,5,3)
histogram(reshape(rel_error_price,prod(size(rel_error_price),1)))
subplot(3,5,4)
heatmap(reshape(mean(rel_error_vola),9,9));
subplot(3,5,5)
