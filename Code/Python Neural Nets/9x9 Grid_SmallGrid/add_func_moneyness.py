...
def calibration_plotter(prediction_invtrafo,X_test,extra_plots = 0):
    error =  100*np.abs((X_test-prediction_invtrafo)/X_test)
    ...
    err_per = 100*np.abs((persistency-persistency_pred)/persistency)
    ...
    if vio_error.size != 0:  
        ...
        plt.boxplot(error,showfliers=False,showmeans=True)
        ax.yaxis.set_major_formatter(mtick.PercentFormatter())
        #plt.yscale("log")
        plt.xticks([1, 2, 3,4,5], ['a','b','g*',"w","h0","persistency"])
        plt.ylabel("Errors")
        ax=plt.subplot(1,3,2)
        plt.boxplot(vio_error,showfliers=False,showmeans=True)
        ax.yaxis.set_major_formatter(mtick.PercentFormatter())
        #plt.yscale("log")
        plt.xticks([1, 2, 3,4,5], ['a','b','g*',"w","h0","persistency"])
        plt.ylabel("Errors parameter violation")
        ax=plt.subplot(1,3,3)
        plt.boxplot(vio_error2,showfliers=False,showmeans=True)
        #plt.yscale("log")
        plt.xticks([1, 2, 3,4,5], ['a','b','g*',"w","h0","persistency"])
        ax.yaxis.set_major_formatter(mtick.PercentFormatter())
    ...
    for i in range(Nparameters+1):
        ...
        plt.hist(error[error[:,i]<np.quantile(error[:,i],0.95),i],bins=100,weights=np.zeros_like(error[error[:,i]<np.quantile(error[:,i],0.95),i]) + 100. / error[error[:,i]<np.quantile(error[:,i],0.95),i].size)
        plt.xlim([0,np.quantile(error[:,i],0.95)])
        ax.yaxis.set_major_formatter(mtick.PercentFormatter())
        ...
        #plt.yscale("log")
        plt.boxplot(error[:,i],showfliers=False,showmeans=True)
        ax.yaxis.set_major_formatter(mtick.PercentFormatter())
    ...
    print("violation error mean in %:",np.mean(vio_error,axis=0))
    print("no violation error mean in %:",np.mean(vio_error2,axis=0))
    print("violation error median in %:",np.median(vio_error,axis=0))
    print("no violation error median in %:",np.median(vio_error2,axis=0))
    print("error mean in %:",err1)
    print("error median in %:",err2)
