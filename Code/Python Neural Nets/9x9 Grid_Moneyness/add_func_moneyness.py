...
from config_moneyness import bound_sum_price,diff_price
...
def rmse_constraint(param):
    def rel_mse_constraint(y_true, y_pred):
            traf_a = 0.5*(y_pred[:,0]*diff_price[0]+bound_sum_price[0])
            traf_g = 0.5*(y_pred[:,2]*diff_price[2]+bound_sum_price[2])
            traf_b = 0.5*(y_pred[:,1]*diff_price[1]+bound_sum_price[1])
