...
for y = 2010:2018
    ...
    params_Q                  = params_Q_mle_weekly(idx:idx+num_-1,:);
    params_P                  = params_P_mle_weekly(idx:idx+num_-1,:);
    sig_unc                 = sigma2_last(idx:idx+num_-1);%(params(:,1)+params(:,2))./(1-params(:,2).*params(:,4).^2-params(:,3));
    idx = idx+num_;
    sig_tmp(weeks,:)        = sig;
    sig0_tmp(weeks,:)       = sig_old;
    sig0_unc_tmp(weeks,:)   = sig_unc;
    vola_tmp(weeks,:)       = vola;
    params_tmp(weeks,:)     = params_Q;
    params_tmp_P(weeks,:)     = params_P;
    name                    = strcat('weekly_',num2str(i+2009),'_mle_opt_h0est_UpdateQ.mat');
    save(name,'sig_tmp','vola_tmp','params_tmp_P','params_tmp','sig0_tmp','sig0_unc_tmp')
end