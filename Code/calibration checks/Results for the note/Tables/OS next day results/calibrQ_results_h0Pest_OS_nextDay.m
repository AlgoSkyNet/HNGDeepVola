...
for cur_num = 1:num_years
    load(['data for tables/h0calibrated/no update/OS OptLL/params_options_', year_nums{cur_num}, '_h0_calibr_noUpd_nextday.mat']);
    %load(['data for tables/h0calibrated/update Q with MLEP params/OS OptLL/params_options_', year_nums{cur_num}, '_h0_calibr_UpdWithMLEPparams_nextday.mat']);
    %load(['data for tables/h0UncVarUpdateQ/no update/OS OptLL/params_options_', year_nums{cur_num}, '_h0_uncVarUpdQ1year_noUpd_nextday.mat']);
    %load(['data for tables/h0UncVarUpdateQ/update Q with MLEP params/OS OptLL/params_options_', year_nums{cur_num}, '_h0_uncVarUpdQ1year_UpdWithMLEPparams_nextday.mat']);
    %load(['data for tables/h0MLEPht/no update/OS OptLL/params_options_', year_nums{cur_num}, '_h0_MLEP_noUpd_nextday.mat']);
    %load(['data for tables/h0UncVarUpdateQ/update Q with MLEP params/OS OptLL/params_options_', year_nums{cur_num}, '_h0_uncVarUpdQ1year_UpdWithMLEPparams_nextday.mat']);
    ...
    for i=1:length(valuesOS)
        if ~isempty(valuesOS{1,i})
            ...
            year_data(cur_num).OptionsLikelihoodNorm(j) = valuesOS{1,i}.optionsLikhngNorm;
            year_data(cur_num).OptionsLikelihood(j) = valuesOS{1,i}.optionsLikhng;
            year_data(cur_num).AIC(j) = valuesOS{1,i}.AIC;
            year_data(cur_num).BIC(j) = valuesOS{1,i}.BIC;
            year_data(cur_num).AICc(j) = valuesOS{1,i}.AICc;
...
mean_OptionsLikelihoodNorm = arrayfun(@(x) mean(x.OptionsLikelihoodNorm), year_data);
mean_AIC = arrayfun(@(x) mean(x.AIC), year_data);
mean_BIC = arrayfun(@(x) mean(x.BIC), year_data);
mean_AICc = arrayfun(@(x) mean(x.AICc), year_data);
mean_MAPE = arrayfun(@(x) mean(x.MAPE), year_data);


FID = fopen('calibrQ_results_h0calNoUpd_h0Pest_calls_OSND_10_18.tex', 'w');
%FID = fopen('calibrQ_results_h0calUpdWithMLEP_h0Pest_calls_OSND_10_18.tex', 'w');
%FID = fopen('calibrQ_results_h0uncVarNoUpd_h0Pest_calls_OSND_10_18.tex', 'w');
%FID = fopen('calibrQ_results_h0uncVarUpdWithMLEP_h0Pest_calls_OSND_10_18.tex', 'w');
%FID = fopen('calibrQ_results_h0MLEPNoUpd_h0Pest_calls_OSND_10_18.tex', 'w');
%FID = fopen('calibrQ_results_h0uncVarUpdWithMLEP_h0Pest_calls_OSND_10_18.tex', 'w');
...
fprintf(FID, '\\toprule \n');
fprintf(FID, '\\multicolumn{10}{c}{{\\bf CALIBRATED PARAMETERS AND $h_0^Q$ON WEDNESDAYS, FROZEN}} \\\\\n');
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf CALIBRATED PARAMETERS ON WEDNESDAYS, $h_0^Q=ht^P$, FROZEN}} \\\\\n');
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf CALIBRATED PARAMETERS ON WEDNESDAYS, $h_0^Q$ IS UNC UNDER P, UPDATED UNDER Q 1 YEAR, FROZEN}} \\\\\n');
...
fprintf(FID, '\\cmidrule(r){1-10} \\\\\n');
fprintf(FID, ' { {\\bf OptLL Norm} }& $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$& $%.4f$& $%.4f$ \\\\\n', mean_OptionsLikelihoodNorm(:)');
fprintf(FID, '\\cmidrule(r){1-10} \\\\\n');
fprintf(FID, ' { {\\bf OptLL} }& $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$& $%.4f$& $%.4f$ \\\\\n', mean_OptionsLikelihood(:)');
fprintf(FID, '\\cmidrule(r){1-10} \\\\\n');
fprintf(FID, ' { {\\bf AIC} }& $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$& $%.4f$& $%.4f$ \\\\\n', mean_AIC(:)');
fprintf(FID, '\\cmidrule(r){1-10} \\\\\n');
fprintf(FID, ' { {\\bf AICc} }& $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$& $%.4f$& $%.4f$ \\\\\n', mean_AICc(:)');
fprintf(FID, '\\cmidrule(r){1-10} \\\\\n');
fprintf(FID, ' { {\\bf BIC} }& $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$ & $%.4f$& $%.4f$& $%.4f$ \\\\\n', mean_BIC(:)');
