...
for cur_num = 1:num_years
    load(['data for tables/h0UncVarUpdateQ/update Q with MLEP params/OS OptLL/params_options_', year_nums{cur_num}, '_h0_uncVarUpdQ1year_UpdWithMLEPparams.mat']);
...
FID = fopen('calibrQ_results_h0UncVarUpdQwithMLEPparams_h0Pest_calls_OS_10_18.tex', 'w');
...
fprintf(FID, '\\toprule \n');
fprintf(FID, '\\multicolumn{10}{c}{{\\bf $h_0^Q$ IS UNC UNDER P, UPDATED UNDER Q 1 YEAR, THEN 1 WEEK AGAIN WITH MLE P PARAMETERS ESTIMATED FROM YEAR -1 TO -11}} \\\\\n');
fprintf(FID, '\\midrule \n');
