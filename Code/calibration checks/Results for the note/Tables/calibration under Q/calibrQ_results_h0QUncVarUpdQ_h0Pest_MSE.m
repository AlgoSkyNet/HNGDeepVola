...
for cur_num = 1:num_years
    load(['data for tables/results calibration h0asUncondVUpdate esth0P/MSE/params_options_', year_nums{cur_num}, '_h0asUncVarUpdQ_MSE_interiorpoint_noYield_m.mat']);
...
FID = fopen('calibrationQ_results_h0QisUncondVarUpdateQ_h0Pest_calls_MSE_2010_2018.tex', 'w');
...
fprintf(FID, '\\toprule \n');
fprintf(FID, '\\multicolumn{10}{c}{{\\bf CALIBRATED PARAMETERS ON WEDNESDAYS WRT MSE, $h_0^Q = \\dfrac{\\omega_0 + \\alpha_0}{1-\\beta_0 - \\alpha_0 \\gamma_0^{*2}}$, WITH $\\omega_0, \\alpha_0, \\beta_0, \\gamma_0^{*2}$ FROM MLE UNDER P AND UPDATED UNDER Q}} \\\\\n');
fprintf(FID, '\\midrule \n');
