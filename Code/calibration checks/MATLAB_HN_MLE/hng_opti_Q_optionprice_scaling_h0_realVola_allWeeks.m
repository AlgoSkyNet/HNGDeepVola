...
for i = unique(weeksprices)
    ...
    elseif multistart_idx
        f_min_raw = @(params) (mean((price_Q(params, data_week, r_cur./252, sig2_0(i))' - data_week(:, 1)).^2));
        ...
        problem = createOptimProblem('fmincon', 'x0', Init_scale.*scaler, ...
            'objective', f_min_raw, 'lb', lb.*scaler, 'ub', ub.*scaler, 'nonlcon', @nonlincon_scale_init, 'options', options_multistart);
        [xxval, fval, exitflag, gsresults, gsvec] = run(ms, problem, 10);
...
Errors2 = zeros(53,1);
for i = 2:53
    params2(i, :) = values{1,i}.hngparams;
    Errors2(i) = values{1,i}.MSE;
end