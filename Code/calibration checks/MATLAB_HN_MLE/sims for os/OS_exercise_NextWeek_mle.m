...
%     load(strcat('/Users/lyudmila/Dropbox/GIT/HenrikAlexJP/Code/calibration checks/MATLAB_HN_MLE/calibration h0calibrated/OptLL/results calibr h0calibarted esth0P OL mp verif/','params_options_',num2str(year_num),'_h0_calibrated_OptLL_interiorpoint_noYield.mat'));
    load(strcat('D:/Github/MasterThesisHNGDeepVola/Code/calibration checks/Calibration MLE P/Results with estimated h0P/','weekly_',num2str(year_num),'_mle_opt_h0est.mat'));
    load(strcat('D:/Github/MasterThesisHNGDeepVola/Code/calibration checks/MATLAB_HN_MLE/calibration h0calibrated/OptLL/results calibr h0calibarted esth0P OL mp verif/','params_options_',num2str(year_num),'_h0_calibrated_OptLL_interiorpoint_noYield.mat'));
    mle = load(strcat('D:/Github/MasterThesisHNGDeepVola/Code/calibration checks/MATLAB_HN_MLE/MLE_P estimation results/Results with estimated h0P/','weekly_',num2str(year_num),'_mle_opt_h0est.mat'));
...
%                 end
            ...
            if ~isempty(values{1,k})
                ...
                total_struc{1,j}.sig_mle= mle.sig_tmp(k);
                total_struc{1,j}.mleQ_params = mle.params_tmp(k,:);
            end
...
struct_ind = 2;
MSE_tot = 0;
mle_tot = load(strcat('D:/Github/MasterThesisHNGDeepVola/Code/calibration checks/MATLAB_HN_MLE/MLE_P estimation results/Results with estimated h0P/','weekly_10to18_mle_opt_h0est_check.mat'));
for year_num = 2010:2018
...
%        params_for_update   = strucPrev.params_Q(1:4);        
        %strucOSCur.hngparams   = strucPrev.hng_parms
        strucOSCur.hngparams   = strucPrev.mleQ_params;
        %strucOSCur.hngparams = mle_tot.params_Q_mle_weekly(my_idx,:);
%         [~, sigma2_vals] = ll_hng_Q_n(params_for_update, logret, r, strucPrev.sig20);
%         sig20 = sigma2_vals(end);
        %sig20 = strucPrev.sig20;
        sig20 = strucPrev.sig_mle;
        %sig20 = mle_tot.sigma2_last(my_idx);
        %disp(my_idx);
        %my_idx =my_idx +1;
    ...
    save(strcat('params_options_',num2str(year_num),'_mle_h0_est_','noUpd.mat'),'valuesOS');
end
