...
for cur_num = 1:num_years
    % Yields
    %load(['data for tables/Yields/Results with estimated h0P rAv/weekly_', year_nums{cur_num}, '_mle_opt_h0est_rAv.mat']);
    %load(['data for tables/Yields/Results with not estimated h0P rAv/weekly_', year_nums{cur_num}, '_mle_opt_noh0est_rAv.mat']);
    %load(['data for tables/Yields/Results with estimated h0P rWeek/weekly_', year_nums{cur_num}, '_mle_opt_h0est_rWeekYield.mat']);
    %load(['data for tables/Yields/Results with not estimated h0P rWeek/weekly_', year_nums{cur_num}, '_mle_opt_noh0est_rWeekYield.mat']);
    %load(['data for tables/Yields/Results with estimated h0P rWeek for Update/weekly_', year_nums{cur_num}, '_mle_opt_h0est_rWeekYield_Unc.mat']);
    % Tbills
    %load(['data for tables/Tbills/Results with estimated h0P rWeek/weekly_', year_nums{cur_num}, '_mle_opt_h0est_rWeekTbill.mat']);
    %load(['data for tables/Tbills/Results with not estimated h0P rWeek/weekly_', year_nums{cur_num}, '_mle_opt_noh0est_rWeekTbill.mat']);
    load(['data for tables/Tbills/Results with estimated h0P rWeek for Update/weekly_', year_nums{cur_num}, '_mle_opt_h0est_rWeekTbill_Unc.mat']);
...
%Yields
%FID = fopen('ready tables/Yields/Results with estimated h0P rAv/estMLEP_results_h0Pest_rAvYield_10_18.tex', 'w');
%FID = fopen('ready tables/Yields/Results with not estimated h0P rAv/estMLEP_results_noh0Pest_rAvYield_10_18.tex', 'w');
%FID = fopen('ready tables/Yields/Results with estimated h0P rWeek/estMLEP_results_h0Pest_rWeekYield_10_18.tex', 'w');
%FID = fopen('ready tables/Yields/Results with not estimated h0P rWeek/estMLEP_results_noh0Pest_rWeekYield_10_18.tex', 'w');
%FID = fopen('ready tables/Yields/Results with estimated h0P rWeek for Update/estMLEP_results_h0Pest_rWeekYield_Unc_10_18.tex', 'w');
%Tbill
%FID = fopen('ready tables/Tbills/Results with estimated h0P rWeek/estMLEP_results_h0Pest_rWeekTbill_10_18.tex', 'w');
%FID = fopen('ready tables/Tbills/Results with not estimated h0P rWeek/estMLEP_results_noh0Pest_rWeekTbill_10_18.tex', 'w');
FID = fopen('ready tables/Tbills/Results with estimated h0P rWeek for Update/estMLEP_results_h0Pest_rWeekTbill_Unc_10_18.tex', 'w');
...
fprintf(FID, '\\toprule \n');
% Yield
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf ESTIMATED PARAMETERS ON WEDNESDAYS MLE UNDER P (10 YEARS), $h_0^P$ IS  ESTIMATED, $r$ IS TAKEN FROM AVERAGE 10 YEARS YIELD}} \\\\\n');
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf ESTIMATED PARAMETERS ON WEDNESDAYS MLE UNDER P (10 YEARS), $h_0^P$ IS NOT ESTIMATED, $r$ IS TAKEN FROM AVERAGE 10 YEARS YIELD}} \\\\\n');
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf ESTIMATED PARAMETERS ON WEDNESDAYS MLE UNDER P (10 YEARS), $h_0^P$ IS ESTIMATED, $r$ IS TAKEN FROM DAY YIELD}} \\\\\n');
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf ESTIMATED PARAMETERS ON WEDNESDAYS MLE UNDER P (10 YEARS), $h_0^P$ IS NOT ESTIMATED, $r$ IS TAKEN FROM DAY YIELD}} \\\\\n');
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf ESTIMATED PARAMETERS ON WEDNESDAYS MLE UNDER P (10 YEARS FROM 1 YEAR BACK), $h_0^P$ IS ESTIMATED, $r$ IS TAKEN FROM TODAY YIELD}} \\\\\n');
% Tbill
...
%fprintf(FID, '\\multicolumn{10}{c}{{\\bf ESTIMATED PARAMETERS ON WEDNESDAYS MLE UNDER P (10 YEARS), $h_0^P$ IS NOT ESTIMATED, $r$ IS TAKEN FROM DAY TBILL}} \\\\\n');
fprintf(FID, '\\multicolumn{10}{c}{{\\bf ESTIMATED PARAMETERS ON WEDNESDAYS MLE UNDER P (10 YEARS FROM 1 YEAR BACK), $h_0^P$ IS ESTIMATED, $r$ IS TAKEN FROM TODAY TBILL}} \\\\\n');
