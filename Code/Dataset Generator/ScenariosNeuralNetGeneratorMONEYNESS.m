...
if strcmp(yieldstype,"szenario")|| strcmp(yieldstype,"testing")
    i_rand = randi(Ninputs,Nsim,1);
end
inv_data = [inv_data(:,1:5),max(inv_data(:,6:end),0)];
interest_rates_test = zeros(Nsim,9);
for aaa=1:9
    interest_rates_test(:,aaa) =0.01*aaa+0.009*rand(Nsim,1);
end
%% Price Calculations
j = 0;
fail1 = 0;
...
for i = 1:Nsim
    ...
    elseif strcmp(yieldstype,"PCA")
        ...
        r_cur(isnan(r_cur)) = 0;
        price         = price_Q_clear([w,a,b,g],data_vec,yieldcurve/252,sig);
    elseif strcmp(yieldstype,"testing")
        int = i_rand(i);
        r_cur = interest_rates_test(i,:);
...
data_vola(:,4+1+Nmaturities+1:4+1+Nmaturities+Nmaturities*Nstrikes) = vola;
...
if saver
    name_file_price = strcat('id_Moneyness_',id,'_data_price_',choice,'_',num2str(size(data_price,1)));
    name_file_vola = strcat('id_Moneyness_',id,'_data_vola_',choice,'_',num2str(size(data_vola,1)));
    name_file_vega = strcat('id_Moneyness_',id,'_data_vega_',choice,'_',num2str(size(data_vega,1)));
    if scenario_cleaner
    ...
    if price_cleaner
       ...
       name_file_vega = strcat(name_file_vega,'_bigprice');
    end
